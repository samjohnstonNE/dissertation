...
export const TransactionProvider = ({ children }) => {
    ...
    const getEthCurrentPrice = () => {
        try {
            ...
            fetch(url)
                ...
                .then((data) => {
                    setEth(["$" + data.result.ethusd + " | " + "ETH/BTC: " + data.result.ethbtc]);
                })
                .catch((err) => {
                    console.log("something went wrong ", err);
                });
    ...
    const checkWalletConnection = async () => {
        try {
            if (!ethereum)
                return alert(
                    "Please install either MetaMask or MathWallet Connect. Icons in the top left are links to the installation pages"
                );
    ...
    const connectWallet = async () => {
        try {
            if (!ethereum)
                return alert(
                    "Please install either MetaMask or MathWallet Connect. Icons in the top left are links to the installation page"
                );
